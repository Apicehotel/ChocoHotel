...
function PlanningSale({ user, onClose, onFlash }) {
  const canEdit =
    user.role === "direzione" || user.role === "sviluppatore";
...
function PlannedDetail({
  ...
  const canTick =
    !done &&
    (isAssigned ||
      ["manutentore", "direzione", "reception", "direzione", "sviluppatore"].includes(
        user.role,
      ));
  const toggleRoom = (r) => {
    if (!canTick) return;
  ...
  const canEdit =
    user.role === "governante"
      ...
      : ["direzione", "manutentore", "reception", "direzione", "sviluppatore"].includes(
          user.role,
        );
  const canOrderPiece =
    (user.role === "manutentore" ||
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              >
                {[
                  ...
                  const sel =
                    k === "golosi" ? areaSubtype === "golosi" : newRole === k;
                  return (
                    <button
                      key={k}
                      onClick={() => {
